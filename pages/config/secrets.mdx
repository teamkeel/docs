...
Every app has secrets - API keys, passwords, credentials etc... Managing secrets can be a bit of a pain so we've built secret management into Keel that's both secure and easy to to use.

import { Callout } from 'nextra/components'

<Callout type="warning" emoji="⚠️">
  For non-sensitive configuration values use [environment variables](/config/envvars).
</Callout>
