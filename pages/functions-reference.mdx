...
[Custom functions](/functions/custom-functions) are arbitrary functions you define in your schema using the `read` or `write` action types. They allow you to execute custom logic that doesn't fit into standard CRUD operations.
...
[Function hooks](/functions/action-hooks) apply to standard CRUD actions (`get`, `list`, `create`, `update`, `delete`) and allow you to modify or extend the behavior of these actions at different stages.
...
The [Models API](/functions/sdk/modelApi) provides a set of methods to interact with your data models. The methods are standard across models, but the actual models and their fields are generated based on your schema.
...
The [Database API](/functions/sdk/databaseApi) allows you to execute raw SQL queries using [Kysely](https://koskimas.github.io/kysely/) as the underlying query builder.
...
The Context API provides access to the request-specific context, including the authenticated user's identity, request headers, and more. Visit our [identity](/identity) and [authentication](/authentication/getting-started) pages for more.
