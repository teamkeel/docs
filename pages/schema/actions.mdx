...
In this example we accept the `name` field as in input to the action but set the `identity` field based on the callers Identity. For more information on Identity see the [Identity docs](/schema/identity).
