...
For SSO Login, make sure to configure the `redirectUrl`.  This URL will be redirected to after the `auth/authorize/{provider}` flow has successfully completed.  An auth code will be provided in querystring, which can then be used at the token endpoint to acquire tokens.
