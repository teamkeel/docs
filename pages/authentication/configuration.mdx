...
For Single Sign-on (SSO) Login, make sure to configure the `redirectUrl`.  Users will be sent here after the `auth/authorize/{provider}` flow has successfully completed. An auth code will be provided in the query parameter `code`, which can then be used at the token endpoint to acquire an access and refresh token. We go into more detail about this on the [SSO Login page](/authentication/sso-login).
