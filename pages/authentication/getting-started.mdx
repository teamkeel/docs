...
You can configure **any OpenID Connect authentication provider** to work with Keel, however we have already preconfigured some of the popular providers for you.  

The table below lists the providers we support out of the box, however **you can configure any OpenID Connect provider you want** using the `oidc` and `oidc` provider type.
...
To use an authentication flow with an external service, you will need to set up a client with each of the third-party providers you have configured. For example, to setup "Login with Google" authentication flow, you'll need to create a [Google client](http://localhost:3000/authentication/providers/google#client-setup). You'll need to do similar things on [Facebook](https://developers.facebook.com/docs/facebook-login/guides/access-tokens/), [GitHub](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authenticating-to-the-rest-api-with-an-oauth-app#registering-your-app), and any other provider you want to use.
