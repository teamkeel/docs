...
Keel, a similarly feature-packed platform (a Swiss army knife, to say the least), is an all-in-one backend platform, allowing you to build backends rapidly.
...
## How ID Token Exchange Works

{/* Raw diagram: https://excalidraw.com/#json=HaQnMavJDR-PouOYV8hQU,-t34iARa26Qf5xDGHreVnQ */}
![A diagram showing the communication flow between Keel, Clerk and the client](https://i.imgur.com/1ERh66n.png)

For this guide, we will use an [ID token authentication flow](https://docs.keel.so/authentication/flows/id-token). Put simply, with the ID token authentication we're able to authenticate the client with just an arbitrary ID token, provided by an authorisation server - Clerk in this case.

The general steps of the flow are:

1. Logging in with the authorisation provider (Clerk)
2. Extracting the `id_token` from the successful token endpoint response.
3. Performing the token exchange with Keel.
4. Using the [access and refresh tokens](https://docs.keel.so/authentication/flows/id-token) to mutate the Keel API.
...
For this guide we will setup a simple Next.js [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) application, that will integrate both Clerk and Keel under the hood.
...
```
<Callout>
The `<ClerkProvider />` must wrap the whole app, followed by the `<SignedIn />` component that renders the content only when the user is signed in, in which the `<KeelProvider />` resides, followed by `<KeelAuthProvider />`. The exact structure is necessary, since the `<KeelAuthProvider />` is dependant on the `<KeelProvider />` and `<ClerkProvider />`.
</Callout>
...
In this guide, you've learned how to integrate Clerk with Keel, using an ID token exchange flow. This guide should give you an overview of how simple it is to utilise third-party authentication providers with Keel.

Continue your authentication journey in Keel with the [overview of authentication](https://docs.keel.so/authentication/getting-started), or see how Keel handles [Single Sign-On (SSO)](https://docs.keel.so/authentication/flows/id-token) or [password-based](https://docs.keel.so/authentication/flows/password) authentication.