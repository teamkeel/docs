...
<Callout type="info" emoji="🚀">
  Very soon, any tests you write and commit to your repository will be run as part of the deployment process.
</Callout>


The Keel testing framework allows you to write tests for all actions defined in your schema with TypeScript, in a completely typesafe manner!

We will run your tests against a short-lived local instance of our runtime specifically designed to enable users to write tests, so there are no nasty surprises when you come to deploy your schema! Data created during test runs will be stored in a local postgres instance on your computer, and will be destroyed at the end of the test run.
...
First thing you will need to do is to run Keel's code generation command in order to dynamically generate TypeScript types for your schema so you can interact with the Keel testing framework in a typesafe manner:
...
The `actions` export contains all of the actions defined in your schema, whether they are built-in actions, or any custom implementations you may have written using the `@function` attribute!
