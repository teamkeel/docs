...
Jobs can be scheduled or manual. Schedluled jobs are run on a repeating, scheduled basis, whereas Manual jobs are executed
...
which allows you to control the  users in your system that can execute them. Jobs only accept permissions defined by [Roles](./permissions#roles).
