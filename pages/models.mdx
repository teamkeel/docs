...
Computed fields can also reference fields in related models. In the case a one-to-many lookup, aggregate functions can be used to perform a calculation across a set of related data. See the example below.
...
model Order {
  fields {
    ...
    // Performing a simple calculation using the total fields on this model
    vat Decimal @computed(order.total * 0.2)
...
- Can only be used on `Decimal`, `Number`, and `Boolean` fields
