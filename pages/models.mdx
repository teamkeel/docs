...
A computed field is type of field on a model defined with the `@computed` attribute and whose value is generated based on the provided expression. The simple example below illustrates how the `vat` field is computed based on multiplying the `total` field by 0.2 and the `totalExcludingVat` field is computed based on subtracting the `vat` amount from `total`. Whenever the `total` field is set, the `vat` and `totalExcludingVat` fields are automatically updated within the same transaction.
...
Computed fields can also reference fields in related models. In the case a one-to-many lookup, aggregate functions can be used to perform an calculation cross a dataset. See the example below.
...
model OrderItem {
  fields {
    ...
    product Product
    quantity Number
    subTotal Decimal @computed(product.price * orderItem.quantity)
  }
}
...
The following table outlines the aggregate functions currently supported in computed expressions in one-to-many lookups.
...
- Only available for `Decimal`, `Number`, and `Boolean` fields at present.
...
- Cannot be set in actions nor in the ModelAPI.
