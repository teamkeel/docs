...
A computed field is type of field which is annotated with the `@computed` attribute and whose value is automaticallygenerated based on the provided expression. 
...
Computed fields can also reference fields in related models. In the case a one-to-many lookup, aggregate functions can be used to perform a calculation across a set if related data. See the example below.
...
- Computed fields are updated within the same transaction as the mutating.
