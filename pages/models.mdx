...
All models have a few built-in fields by default - `id`, `createdAt`, and `updatedAt`, so although not something you are likely to do a lot it is valid to define a model with no fields block. This means the following is a valid model.
...
At the model level, you can also specify uniqueness if you have a combination of multiple fields that need to be unique accross all instance of a model.
...
Previously, we discussed how a models built-in fields have default values that are used when a record is created. You can also add default values to your own fields using the `@default` attribute. This attribute accepts a single argument, which is the value you want to use as the default
