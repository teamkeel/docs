...
We can use this a **list** action to perform queries like:
...
These docs talk about **actions**, which is the general term for both operations (implemented by the Keel runtime) and functions (implemented by you using code). There is no difference between how operations and functions are surfaced in GraphQL.
