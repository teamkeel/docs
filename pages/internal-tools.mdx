...
### Toolbar
Each tool may have [additional actions](/internal-tools/linked-actions) that are automatically linked for you. You will also see more options to favourite the action so that it stays in the sidebar for easy access, and open the action in the API explorer, which will show you examples of the types of API endpoints you can use in your own apps.

### Input field options
The input field components have been configured to match the input type you have written in your schema. 
...
- For boolean fields, a yes/no toggle button will appear. You can set these to null if optional or clear the field and leave it unset.
