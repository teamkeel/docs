...
Events and subscribers can be a crucial part of your application, and so Keel has made it possible for you to test their behaviour by providing supporting for them in [tests](/testing). 

<Callout type="info" emoji="ℹ️">
Events in tests are sychronous. This means that you can always expect all subscriber functions to have completed executing before your action or function call returns.  This has been done to make testing events easier.
</Callout>
...
It is also possible to simply run your subscriber function directly, without the need to trigger an event. This is a useful way to isolate and test just the subscriber's implementation.
