...
If a subscriber function fails to execute, then the event will return to the queue and will be reprocessed again after 80 seconds.  This will repeat for 3 attempts, afterwhich the event will be deleted.
...
| Number of retry attempts for a failing subscriber function | 3 attempts |
| Backoff duration betweeen retry attempts                   | 80 seconds |
| Event chain depth limit                                    | 15         |
| Event message size                                         | 256 kB     |
