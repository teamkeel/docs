...
On the internal tool page, when these fields are displayed, Keel will automatically link that id to the `get` action for that related model (if the `get` action exists in the schema).
