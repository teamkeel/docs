...
const email = await ctx.ui.page("customer", {
  ...
  content: [
	ctx.ui.elements.input.text("name"),
    ctx.ui.elements.input.text("email", {label: "Email address"}),
  ],
});
```
...
export default RefundOrder(async (ctx, inputs) => {
  ...
  const customer = await ctx.ui.page("customer", {
    ...
    content: [ctx.ui.elements.input.text("email", { label: "Customer email" })],
  });
  ...
  const order = await ctx.ui.page("select order", {
    ...
    content: [
      ctx.ui.elements.select.table("orderId", {
        label: "Select order",
