...
Permission rules apply to both operations and functions, and you can add permission rules to specific functions too. The only exception to this is for `read` and `write` functions - schema defined permission rules do not apply to these functions and so you must implement any permissions logic in your code. See the docs on [custom functions](/functions#permissions) for more info.