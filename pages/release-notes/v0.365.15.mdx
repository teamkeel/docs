...
We are excited to announce the release of Keel `v0.365.15`. This update brings breaking changes to our Keel schema language, a new feature and bug fixes aimed at enhancing user experience and productivity.

## Breaking Change

We are excited to introduce the **Hooks API**, which is a complete revamp of functions. You will need to restructure your existing functions to use [Hooks](link to documentation). With **Hooks** we now write most of the implementation of your function for you, which should mean less code for you to write.
...
You will need to upgrade your Keel CLI version to see this change reflect and we have written a [Migration Guide](LINK) that you can follow to upgrade your existing Keel schemas.

Once you have followed the **Migration Guide** [LINK] to update your schemas locally, you can push the commit, and we will rebuild your updated schema and deploy into production.
...
To upgrade to Keel `v0.365.15`, please run the following command:

```bash
brew upgrade keel
```

Read the migration Guide for Hooks:  (LINK)
