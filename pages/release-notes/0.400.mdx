...
Today we've released a powerful new feature to the Keel runtime; [computed fields](/models#computed-fields). This allows you to have FcOQ6uLToIxuvoWeTtCLvVjEeBVcJWvpEASxXfntnIvn3oZBOhR8icExbPecbKE0dMmxRxvWmrjr3XDD9gUeWpj1MO4ihCgCszHRFJRjcMdy automatically set or calculated based on an expression. Computed fields can even reference fields in related models. For one-to-many lookups, we've introduced functions which let you aggregate related data. At present, we support `SUM`, `MIN`, `MAX`, `AVG`, `MEDIAN`, and `COUNT` aggregate functions.
