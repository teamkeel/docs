...
We have just released two exciting features to Keel.  These have been shipped with version `0.393` of the CLI.
...
We have changed the shape of the arguments to `authenticateWithPassword()`, `authenticateWithIdToken()` and `authenticateWithSingleSignOn`. Previously, they had accepted multiple arguments (for example, `authenticateWithPassword(email, password)`), however they will now take accept the arguments as fields of an object. 
...
We have decideds to change our approach to errors on the generated client and _not throw them_. This means that most of the `auth` functions can now return errors.
