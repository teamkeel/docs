...
The [application](https://saas-starter-ui.vercel.app/) we are building a backend for is a SaaS application that is similar to the popular project management tool called [Notion](https://www.notion.so) and has the following features:
...
So with these, we know what we need to cater for on the backend with Keel.

## Thinking in Keel

Everything in Keel is a model, which is built around your data. Models are defined in the a `.keel` file, they can be named anything and are also fully typed. We need to define the following models for the application based on the above information we know about the application:
