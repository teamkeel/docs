...
Before we work on the `Team` model, let’s talk a bit about Keel’s permission system, specifically the `in` operator. Permissions are crucial for any SAAS application and Keel caters for permissions out of the box.
...
<Callout type="info" emoji="💡">
  Please note that the `in` operator don’t filter through permissions. When used in a permission, they just return a permission error if the response data contains anything the user can’t access
</Callout>
