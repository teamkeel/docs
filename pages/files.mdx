...
Being able to work with files are essential for many businesses. The Keel `File` field type provides an easy and managed way to work with files in your Keel application. As with any field type, it can be used in models, actions, functions, jobs and even in subscriber functions. 

Take the schema below for example.  Here we have defined a `Product` model which have two properties; its title and an file which would, presumably, store a photograph of the product.
...
<Callout type="info">
  At present, files cannot exceed 4MB in size, however there is no constraint on files types and
</Callout>

### API inputs

When used as an input, files are being passed as [data URLs](https://developer.mozilla.org/en-US/docs/Web/URI/Schemes/data); the OpenAPI schema generated for an `File` field will be `string` with format type `data-url`.

This data URL must also include a `name` parameter which stipulates the filename for the file. e.g. `data:image/png;name=product.png;base64,iVBORw0KGgoAAAANSUhEUg.....`

### Response

When an API response of an action or function includes the `File` type (i.e. it might form part of the model being returned or defined in the custom function message), the format in which these fields are returned is as follows:
...
The `url` field provides a shoter-term presigned URL to the file, which will expire after 24 hours. This URL is regenerated each time a file response is returned.
...
The `File` class allows you to read files in functions. For example, this may be useful for importing bulk from a CSV file. See the code sample below.
...
model User {
    ...
    actions {
        write writeBulk(csv: File) returns (Any)
    }
}
```
  </Tab>
    <Tab>
```ts
import { WriteBulk, models } from '@teamkeel/sdk';

export default WriteBulk(async (ctx, inputs) => {
...
It is also possible to construct a new file from scratch within your function, to either be stored or just returned from the API. 
...
model Post {
  ...
  actions {
    read createFromImage(ImageInput) returns (Any)
  }
}
```
  </Tab>
