...
When creating a `@unique` relationship between some model and `Identity`, as with many of the examples above, you will then be able to traverse from `Identity` to your model in expressions which can be a very powerful modelling tool.  For example, the following will become possible.
