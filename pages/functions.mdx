...
Functions are [Actions](/actions) that are defined in your schema but implemented using code. We currently support TypeScript for writing functions.

Your function code should be located in the `functions` directory of your project, with each function in its own file named the same as the function in the schema. So if you have a function called `doTheThing` in your schema, the code for this function would be located at `functions/doTheThing.ts`. 
...
Your functions receive three arguments.
...
export default MyFunction((inputs, api, ctx) => {
  ...
  ctx.api.headers.set("X-My-Other-Header", "1234");
});
```
...
In much the same way any secrets you've defined in your `keelconfig.yaml` file will be available by using `ctx.secrets` which is also typed. As secrets are sensitive values they are not set as environment variables and are only accessible by using `ctx.secrets`.
