...
<Callout type="info" emoji="ℹ️">
  Although the `afterWrite` hook can be used to perform side-effects after a write, in many cases it will be better to use an [event](/events). The key difference is that the `afterWrite` hook happens as part of your action and so will affect the response time of your action, whereas an event happens _asynchronously_.
</Callout>
...
export default DeleteProduct({
  async afterWrite(ctx, inputs, data) {
    await sendSlackMessage("#products", `${data.title} (${data.id}) has been deleted!`);
  },
});
```
