...
`read` and `write` actions do not have a default implementation. They are implemented by you!
...
`beforeWrite` expects the hook to return an object satisfying the type of the model the hook is defined for. 
