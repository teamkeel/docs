...
export default MyFlow({}, async (ctx, inputs) => {
    // This step will execute and persist its data
    const result1 = await ctx.step("process data", async () => {
        return { value: 42 };
    });

    // This step will execute and persist its data
    const result2 = await ctx.step("validate result", async () => {
        return { isValid: true };
    });

    // This step will NOT execute again - it will return the persisted data from the first "process data" step
    const result3 = await ctx.step("process data", async () => {
        return { value: 100 }; // This code will never run
    });

    // result1 and result3 will be the same object: { value: 42 }
});
```
...
```ts
import { DispatchOrder, models, OrderStatus, DeliveryStatus } from "@teamkeel/sdk";
