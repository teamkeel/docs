...
Authentication is simple with the generated client because you don't need to worry about access tokens and refresh tokens at all.  Making authorized calls to your API happens under the hood. It also automatically refreshes your expiring access tokens.

Once authenticated, the client stores the active session in memory, which means that when a new instance of the client is created the session will be lost and authentication will need to take place again.  However, you can easily implement your own token persistence in order to retain authentication over a longer period and between browser sessions.
...
With [ID Token authentication](/authentication/flows/id-token), you would have already configured the 3rd-party authentication provider correctly in your `keelconfig.yaml` and you would have performed the necessary authentication flow with that provider in order to acquire an ID Token.  Once you have this ID Token, you can simple authenticate as follows:
...
And then configuring the custom token store with your client.
