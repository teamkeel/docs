...
await keel.auth.authenticateWithSingleSignOn({ code: authCode });
```
...
The response from a successful `authenticateWithPassword()`, `authenticateWithIdToken()`, or `authenticateWithSingleSignOn()` includes the field  `identityCreated`.  This will be `true` when the identity was created as a result of authentication - i.e. the identity has signed up.
